...
p_day <- ggplot2::ggplot(df) +
  ...
  ggplot2::geom_pointrange(ggplot2::aes(x = pars, y = median,
                           ...
                           shape = 21, fill = "white", size = 0.5, stroke = 1, linewidth = 1) +
...
p_beep_all_day <- ggplot2::ggplot(df) +
  ...
  ggplot2::geom_segment(ggplot2::aes(x = -Inf, xend = -Inf, y = -0.8, yend = 0.4),
                        linewidth = 0.5, lineend = "square") +
  ggplot2::geom_segment(x = 1, xend = 10, y = -Inf, yend = -Inf,
                        linewidth = 0.5, lineend = "square")

ggplot2::ggsave("example/results/beep_model/estimates_beep_cross.pdf", p_beep_all_day, 
                width = 5, height = 3)
...
ggplot2::ggsave("example/results/combi_model/beep_means_combi.pdf", p_combi_mu, 
                width = 5, height = 3)

#### beep variances

df_stan <- res_stan[, c("2.5%", "50%", "97.5%")]
df_stan <- df_stan[c(22:31), ]
colnames(df_stan) <- c("lower", "median", "upper")
pars <- rownames(df_stan)
pars_df <- data.frame(pars = pars)
df_stan <- cbind(df_stan, pars_df)
...
ggplot2::ggsave("example/results/combi_model/beep_vars_combi.pdf", p_combi_var, 
                width = 5, height = 3)
