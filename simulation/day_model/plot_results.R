...
p <- ggplot2::ggplot(df_plot_long) +
  ggplot2::geom_hline(data = df_axis, ggplot2::aes(yintercept = hline),
                      linewidth = 0.5, alpha = 0.2) +
  ggplot2::geom_line(ggplot2::aes(x = days, y = value, colour = beeps,
                                  group = interaction(beeps, software),
                                  linetype = software), linewidth = 1) +
  ggplot2::geom_point(ggplot2::aes(x = days, y = value, colour = beeps,
                                   fill = software, shape = software),
                      size = 2, stroke = 1) +
  ggplot2::scale_shape_manual(values = c(19, 21)) +
  ggplot2::scale_fill_manual(values = c("transparent", "white")) +
  ggplot2::scale_colour_manual(values = cols) +
  ggplot2::facet_grid(cols = ggplot2::vars(par),
                      rows = ggplot2::vars(diagnostic),
                      scales = "free_y", labeller = ggplot2::label_parsed) +
  ggplot2::labs(x = "Days", colour = "Beeps", fill = "Software",
  ...
  ggplot2::theme(
    ...
    strip.text = ggplot2::element_text(margin = ggplot2::margin(0, 5, 0, 5),
                                       size = 12),
  ...
  ggplot2::guides(linetype = ggplot2::guide_legend(
    override.aes = list(size = 1.5, linewidth = 0.5))
  ) +
...
# only intercepts (beep means)
df_plot <- subset(df, grepl("ic_m", par))

# make long format data frame with factors for plotting
df_plot$beeps <- as.factor(df_plot$beeps)
df_plot$days <- as.factor(df_plot$days)
...
# axis limits for each facet
df_axis <- data.frame(diagnostic = unique(df_plot_long$diagnostic),
                      ymin = c(-0.03, 0, .92), ymax = c(0.03, 0.5, 1),
                      hline = c(0, 0, 0.95))
