...
df_diagnostics <- data.frame(matrix(NA, nrow = 0, ncol = 6))
for (days in n_days) {
  for (beeps in n_beeps) {
    t_read <- system.time({
      ...
      df_diagnostics <- rbind(df_diagnostics, diag_stan)
...
saveRDS(df_diagnostics, file = "simulation/beep_model/simulation_results2.rds")
